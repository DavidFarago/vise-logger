...
async def configure_log_dir() -> str:
    ...
    request_context.session.send_progress(marker)
    
    find_result = await asyncio.to_thread(find_log_file_with_marker, marker)

    if find_result:
        log_file_path, _ = find_result
        return f"Log directory found and verified: {log_file_path.parent}"
    else:
        return "Could not find a verifiable log directory."
...
async def send_session(stars: float, comment: str = "") -> str:
    ...
    request_context.session.send_progress(
        f"Rated session at {timestamp}: {stars} stars. Storing session asynchronously on server."
    )
    ...
    success_message = f"Session {server_session_id} successfully uploaded to server."
