...
def detect_log_directory(found_path: Path) -> Path:
    """
    Detects the log directory based on the number of files with the same suffix.
    """
    directory = found_path.parent
    suffix = found_path.suffix
    if suffix:
        try:
            files_with_same_suffix = [f for f in directory.iterdir() if f.suffix == suffix]
...
def find_log_file_with_marker(marker: str) -> Optional[Tuple[Path, str]]:
    ...
    for loc in unverified_locations:
        ...
        if expanded_path.exists() and expanded_path.is_dir():
            found_path = search_directory(expanded_path, marker)
            if found_path:
                found_path = update_or_add_location(found_path, loc.get("tool", "unknown"))
                return found_path, loc.get("tool", "unknown")
    ...
    logging.info("Stage 4: Searching the rest of the hard drive. This may take a while.")
    drives = [Path('/')]
    if platform.system() == "Windows":
        drives.extend([Path(f"{chr(drive)}:\\") for drive in range(ord('A'), ord('Z') + 1) if Path(f"{chr(drive)}:\\").exists()])
